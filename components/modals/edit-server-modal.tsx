...
export type EditServerFormSchemaType = z.infer<typeof editServerFormSchema>;
...
function EditServerModal() {
  ...
  const form = useForm<EditServerFormSchemaType>({
  ...
  const onSubmit = async (data: EditServerFormSchemaType) => {
    try {
      if (!server) {
        return;
      }

      await axios.patch(apiPaths.editServer(server.id), data);
  ...
  return (
    <Dialog
      open={isOpen && type === ModalType.editServer}
