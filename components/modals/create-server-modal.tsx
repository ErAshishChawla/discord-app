...
export type CreateServeFormSchemaType = z.infer<typeof createServerFormSchema>;
...
function CreateServerModal() {
  ...
  const form = useForm<CreateServeFormSchemaType>({
  ...
  const onSubmit = async (data: CreateServeFormSchemaType) => {
    try {
      const response = await axios.post(apiPaths.createServer(), data);
      const responseData = response.data as Server;

      form.reset();
      router.push(paths.specificServer(responseData.id));
      router.refresh();
      onClose();
    } catch (error) {
      console.log(error);
  ...
  return (
    <Dialog
      open={isOpen && type === ModalType.createServer}
                  ...
                  render={({ field }) => {
                    return (
                      <FormItem>
                        <FormControl>
                          <FileUpload
                            endPoint={ApiEndpoints.serverImage}
                            value={field.value}
